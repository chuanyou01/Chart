...
void CEmail::Char2Base64(char* pBuff64,char* pSrcBuff,int iLen)  
{  
	//1   1   1   1   1   1   1   1  
...
void CEmail::AddFilePath(char * pcFilePath)//添加附件路径  
{  
	if(pcFilePath==NULL)  
	{  
		return;  
...
void CEmail::DeleteFilePath(char* pcFilePath)//删除附件路径  
{  
	char* temp;  
	for (list<char*>::iterator it = m_pcFilePathList.begin();
		it!=m_pcFilePathList.end(); it++)
...
void CEmail::DeleteAllPath(void)  
{  
	m_pcFilePathList.clear();
}  

int CEmail::GetFileData(char* FilePath)  
{  
	m_pcFileBuff=NULL;  
	if(FilePath==NULL)  
	{  
	...
	fread.open(FilePath);
	fread.seekg(0, ios_base::end);
	len = fread.tellg();

// 	m_pcFileBuff = new char[len+1];
// 	memset(m_pcFileBuff, 0)
// 
// 
// 	USES_CONVERSION;  
...
void CEmail::GetFileName(char* fileName,char* filePath)  
{  
...
bool CEmail::CReateSocket(SOCKET &sock)  
{  
...
bool CEmail::SendHead(SOCKET &sock)  
{  
	int rt;  
	memset(m_cSendBuff,0,sizeof(m_cSendBuff));  
...
bool CEmail::SendTextBody(SOCKET &sock)  
{  
	int rt;  
	memset(m_cSendBuff,0,sizeof(m_cSendBuff));  
...
bool CEmail::SendEnd(SOCKET &sock)  
{  
	sprintf_s(m_cSendBuff,"--INVT--\r\n.\r\n");  
	send(sock,m_cSendBuff,strlen(m_cSendBuff),0);  
...
bool CEmail::SendMail(sMailInfo &smailInfo)  
{  
